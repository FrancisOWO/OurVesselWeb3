...
def sc_install(filename):
	filepath = "./SmartContract/" + filename
	sc = open(filepath, 'rb').read()
	sc_encode = base64.b64encode(sc)
	# url
	url = f"{URL_PREFIX}/v2/tx/sc/install"
	...
	data = {
		"category": "wvm",
		"file": str(sc_encode, 'utf-8')
	}
	# 发送请求
	res = requests.post(url=url, params=params, data=json.dumps(data))
	if res.status_code != 200:
		print(res)
		return None
	'''
...
def sc_invoke(name, method, args):
	url = f"{URL_PREFIX}/v2/tx/sc/invoke"
...
def sc_query(name, method, args):
	url = f"{URL_PREFIX}/v2/tx/sc/query"
